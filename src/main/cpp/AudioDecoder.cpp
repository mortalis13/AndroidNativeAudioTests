  ...
  while (this->enabled) {
    if (this->playing) {
      ...
      if (audioPacket->stream_index == audioStream->index && audioPacket->size > 0) {
        ...
        processFrame(buffer, frame_count);
...
void AudioDecoder::processFrame(uint8_t* buffer, int32_t numFrames) {
  if (this->streamWriter) {
    this->streamWriter->writeAudio(buffer, numFrames);
  }
  else {
    int pushedBytes = 0;
    int64_t bytesToWrite = numFrames * this->channelCount * sizeof(float);
